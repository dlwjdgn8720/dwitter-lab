...
insert into portfolio(header, home, about, skills, `work`, testimonials, footer, create_at)
    ...
    SELECT * 
      FROM portfolio 
      WHERE JSON_CONTAINS(work->'$.projects', JSON_QUOTE('pid'), '$');
      
	SELECT * 
	FROM portfolio 
	WHERE 'pid' MEMBER OF (work->'$.projects[*].pid');
    
    
    SELECT project_item, pid
	FROM portfolio,
    ...
    SELECT * 
	FROM portfolio 
	WHERE JSON_SEARCH(work->'$.projects[*].pid', 'one', 'pid') IS NOT NULL;
    
    SELECT * 
	FROM portfolio 
	WHERE JSON_CONTAINS(work->'$.projects', JSON_OBJECT('pid', ?));
    
    SELECT * 
	FROM portfolio 	
	WHERE JSON_OVERLAPS(work->'$.projects[*].pid', JSON_ARRAY('pid'));
